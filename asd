...
:root {
  ...
  --Settings-Background: var(--Chat-Background);
  --Callout-Background: var(--Chat-Background);
  ...
  --Emote-Popout-Background: var(--Chat-Background);
...
body {
  ...
  background-size: 2000% 1600%;
  animation: bgCycle 22s steps(320) infinite;
}
...
body::before {
  ...
  top: 50%;
  left: 50%;
  width: 640px;  /* Adjust the frame width */
  height: 360px; /* Adjust the frame height */
  transform: translate(-50%, -50%);
  background: url("https://raw.githubusercontent.com/Ceikros/CeiCss/main/Sprites/Fleeting_Fireworks_sprite_uncompressed.jpg") no-repeat;
  background-size: 12800px 4320px; /* Adjust based on the actual sprite dimensions */
  animation: frameCycle 1s steps(16) infinite; /* 1 second for 16 frames */
  pointer-events: none;
}

@keyframes frameCycle {
  from { background-position: 0 0; }
  to   { background-position: -10240px 0; } /* Adjust this value based on your full frames width */
}
